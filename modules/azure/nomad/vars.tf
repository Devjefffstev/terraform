...
variable "environment" {
  type        = string
  description = <<EOF
(Optional) Identifies the deployment stage/environment of the resource. 
Standard Abbreviations: sbx, dev, int, prod. Why: Supports lifecycle separation and avoids cross-env conflicts.
EOF
  default     = "dev"
}
variable "app_function_chatam" {
  type        = string
  description = <<EOF
(Optional) Denotes the application name, microservice, or infrastructure purpose. Where multiple words are needed to describe an app/function, they should be separated with hyphens.
Examples: core (Core services), api (API service), debt-db (Database), web (Web front-end)
Why: Differentiates resources based on business logic or function.
EOF
...
variable "nsg_rules" {
  type = map(object({ access = string
    ...
    timeouts = optional(object({ create = optional(string)
      delete = optional(string)
      read   = optional(string)
  update = optional(string) })) }))
  description = "(Optional) One or more security rules to be associated with the Network Security Group."
  default     = {}
